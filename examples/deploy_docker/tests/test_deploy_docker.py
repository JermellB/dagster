...
def test_deploy_docker():
    with docker_service_up(file_relative_path(__file__, "../from_source/docker-compose.yml")):
        ...
        while True:
            ...
            try:
                sanity_check = safe_requests.get("http://{dagit_host}:3000/dagit_info".format(dagit_host=dagit_host)
                )
        ...
        res = safe_requests.get("http://{dagit_host}:3000/graphql?query={query_string}".format(
...
def _wait_for_run_status(run_id, dagit_host, desired_status):
    ...
    while True:
        ...
        run_res = safe_requests.get("http://{dagit_host}:3000/graphql?query={query_string}&variables={variables}".format(
                dagit_host=dagit_host,
